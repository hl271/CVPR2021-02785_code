...
def add_runs_pth(runs, s):
    return s.split(".")[0]+"-"+str(runs)+".pth"

def add_runs_pkl(runs, s):
    return s.split(".")[0]+"-"+str(runs)+".pkl"

def all_jobs():
    jobs = []
    for runs in range(10, 110, 10):
        jobs.append(runs)
    return jobs

def run_job(gpu, runs):
    ...
    accuracy, p_value = analysis(opt.iv,
                                 opt.offset,
                                 opt.fold,
                                 add_runs_pth(runs, opt.eeg_dataset),
                                 add_runs_pth(runs, opt.splits_path),
                                 n_classes,
                                 classes,
                                 opt.classifier,
                                 opt.batch_size,
    ...
    results[(opt.classifier,
             ...
             length,
             channel,
             runs)] = (accuracy, p_value)
    f = open(add_runs_pkl(runs, opt.results_file), "wb")
    pkl.dump(results, f)
    f.close()
